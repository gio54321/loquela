...
/// Preprocessed lookup table for byte-level logical right shift.
///
...
///
/// The carry propagates into the *higher* byte (byte i carry flows into byte i-1),
/// which is the opposite direction from SLL.
///
/// The `"byte_srl"` bus schema is `(byte_val, bit_shamt, shifted_byte, carry_byte)`.
#[derive(Clone)]
pub struct ByteShiftRightAir {
    num_lookups: usize,
}

impl ByteShiftRightAir {
    pub fn new() -> Self {
        Self { num_lookups: 0 }
    }
}

impl Default for ByteShiftRightAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F: Field> BaseAir<F> for ByteShiftRightAir {
    fn width(&self) -> usize {
        1
    }
    ...
    fn preprocessed_trace(&self) -> Option<RowMajorMatrix<F>> {
        ...
        for byte_val in 0u32..256 {
            for bit_shamt in 0u32..8 {
                let shifted_byte = byte_val >> bit_shamt;
...
impl<AB: AirBuilder> Air<AB> for ByteShiftRightAir
where
...
impl<F: Field> LookupAir<F> for ByteShiftRightAir {
...
/// Build the main trace for `ByteShiftRightAir`.
///
...
pub fn build_trace<F: Field>(multiplicities: &[F]) -> RowMajorMatrix<F> {
    assert_eq!(
        ...
        "ByteShiftRightAir requires exactly {NUM_PREPROCESSED_ROWS} multiplicities"
    );
