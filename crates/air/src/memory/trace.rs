...
mod tests {
    ...
    fn ops_are_sorted() {
        ...
        assert_eq!(row(&trace, 0).memory_type, f(0));
        assert_eq!(row(&trace, 0).address, limbs(5));
        assert_eq!(row(&trace, 0).timestamp, f(1));
        assert_eq!(row(&trace, 1).memory_type, f(0));
        assert_eq!(row(&trace, 1).address, limbs(5));
        assert_eq!(row(&trace, 1).timestamp, f(3));
        assert_eq!(row(&trace, 2).memory_type, f(1));
        assert_eq!(row(&trace, 2).address, limbs(10));
        assert_eq!(row(&trace, 2).timestamp, f(2));
        assert_eq!(row(&trace, 3).memory_type, f(1));
        assert_eq!(row(&trace, 3).address, limbs(10));
        assert_eq!(row(&trace, 3).timestamp, f(5));
    }
    ...
    fn read_op_has_same_read_write() {
        let ops = vec![MemoryOperation::Read {
            memory_type: MemoryType::Register,
            address: 2,
            timestamp: 0,
            value: 42,
    ...
    fn write_op_splits_old_and_new() {
        let ops = vec![MemoryOperation::Write {
            memory_type: MemoryType::Register,
            address: 2,
            timestamp: 0,
            old_value: 7,
    ...
    fn end_to_end_two_addi() {
        ...
        assert_eq!(row(&trace, 0).address, limbs(0));
        assert_eq!(row(&trace, 0).read, limbs(0));
        assert_eq!(row(&trace, 0).write, limbs(0));
        assert_eq!(row(&trace, 1).address, limbs(1));
        assert_eq!(row(&trace, 1).read, limbs(0));
        assert_eq!(row(&trace, 1).write, limbs(5));
        assert_eq!(row(&trace, 2).address, limbs(1));
        assert_eq!(row(&trace, 2).read, limbs(5));
        assert_eq!(row(&trace, 2).write, limbs(5));
        assert_eq!(row(&trace, 3).address, limbs(2));
        assert_eq!(row(&trace, 3).read, limbs(0));
        assert_eq!(row(&trace, 3).write, limbs(8));
