...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut DecodeColumns<F>, pc: u32, word: u32) {
    ...
    row.instr_type_packed = if is_xori {
        ...
        F::from_u64(InstructionId::Slt as u64)
    } else if is_sltu {
        F::from_u64(InstructionId::Sltu as u64)
    } else if is_slti {
        F::from_u64(InstructionId::Slti as u64)
    } else if is_sltiu {
        F::from_u64(InstructionId::Sltiu as u64)
    } else if is_lui {
        ...
        F::from_u64(InstructionId::Beq as u64)
    } else if is_bne {
        F::from_u64(InstructionId::Bne as u64)
    } else if is_blt {
        F::from_u64(InstructionId::Blt as u64)
    } else if is_bge {
        F::from_u64(InstructionId::Bge as u64)
    } else if is_bltu {
        F::from_u64(InstructionId::Bltu as u64)
    } else if is_bgeu {
        F::from_u64(InstructionId::Bgeu as u64)
    } else {
