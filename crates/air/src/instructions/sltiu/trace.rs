...
use super::air::{SltiuColumns, NUM_SLTIU_COLS};
...
fn extract_sltiu_steps(steps: &[ExecutionStep]) -> Vec<SltiuStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rd, rs1, imm) = match step.instruction {
                Instruction::SltiuI { rd, rs1, imm } => (rd, rs1, imm),
                _ => return None,
            };
            let (timestamp, rs1_value, old_rd_value) = match step.memory_ops.as_slice() {
                [MemoryOperation::Read {
                    timestamp,
                    value: v1,
                    ..
                }, MemoryOperation::Write { old_value, .. }] => (*timestamp, *v1, *old_value),
                _ => return None,
            };
            Some(SltiuStep {
                pc: step.state.pc,
...
/// Compute borrow chain for rs1 - imm_se.
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut SltiuColumns<F>, step: &SltiuStep) {
    ...
    // The 12-bit unsigned immediate (bits 20..31 of the instruction word).
    // imm is stored sign-extended in SltiuI, but the decode bus sends the raw 12-bit unsigned value.
    let imm_u12 = (step.imm as u32) & 0xFFF;
    row.imm = F::from_u64(imm_u12 as u64);

    // Sign-extended immediate (as u32).
    let imm_se = step.imm as u32;

    // High nibble = bits 8..11 of the 12-bit immediate.
    let imm_high_nibble = (imm_u12 >> 8) & 0xF;
    row.imm_high_bits = [
        F::from_u64((imm_high_nibble & 1) as u64),
        F::from_u64(((imm_high_nibble >> 1) & 1) as u64),
        F::from_u64(((imm_high_nibble >> 2) & 1) as u64),
        F::from_u64(((imm_high_nibble >> 3) & 1) as u64),
    ];
    ...
    let (diff, borrows) = u32_sub_borrows(step.rs1_value, imm_se);
    row.diff_bytes = diff.map(|b| F::from_u64(b as u64));
    row.borrow = borrows.map(|b| F::from_u64(b as u64));

    let lt_result = if step.rs1_value < imm_se {
        F::ONE
    } else {
        F::ZERO
    };
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut SltiuColumns<F>) {
    *row = SltiuColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rd: F::ZERO,
...
/// Build the SLTIU execution trace from the VM execution steps.
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    steps: &[ExecutionStep],
) -> DenseMatrix<F> {
    let sltiu_steps = extract_sltiu_steps(steps);
    assert!(!sltiu_steps.is_empty(), "no SLTIU steps found in trace");

    let num_rows = sltiu_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_SLTIU_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<SltiuColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(sltiu_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(sltiu_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_SLTIU_COLS)
}
