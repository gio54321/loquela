...
/// One row per SRAI instruction execution.
///
/// SRAI: `rd = (rs1 as i32) >> (imm & 0x1F)` (arithmetic right shift, fills with sign bit).
///
/// I-type encoding: opcode=0x13, funct3=0x5, imm[11:5]=0b0100000 (bit 30 = 1).
...
///
/// Within-byte shift: same as SRLI — for each byte `i` of rs1:
///   `(rs1_bytes[i], bit_shamt) -> (shifted_bytes[i], carry_bytes[i])` via `"byte_srl"` bus.
///
/// Whole-byte shift: same arrangement as SRLI, but vacated high bytes are filled with
...
pub struct SraiColumns<F> {
    ...
    /// `(0xFF << (8 - bit_shamt)) & 0xFF` for `bit_shamt > 0`, else 0. This is the
    /// "top-bits mask" for arithmetic shift sign extension within a byte.
    /// Constrained as the carry output of the same `byte_srl(0xFF, bit_shamt)` lookup.
    pub srl_ff_carry: F,
...
pub const NUM_SRAI_COLS: usize = size_of::<SraiColumns<u8>>();

impl<T> Borrow<SraiColumns<T>> for [T] {
    fn borrow(&self) -> &SraiColumns<T> {
        debug_assert_eq!(self.len(), NUM_SRAI_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<SraiColumns<T>>() };
...
impl<T> BorrowMut<SraiColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut SraiColumns<T> {
        debug_assert_eq!(self.len(), NUM_SRAI_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<SraiColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for SraiAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for SraiAir {
    fn width(&self) -> usize {
        NUM_SRAI_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for SraiAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &SraiColumns<AB::Var> = main.current_slice().borrow();
        ...
        // Since the decode AIR enforces imm[11:5]=0b0100000 for SRAI, the field imm column
        // on the decode bus carries only imm[4:0] (the shamt), which is in 0..31.
        builder.assert_eq(
            local.imm.clone(),
...
impl<F: Field> LookupAir<F> for SraiAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &SraiColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        // Assert the decoded instruction is SRAI with (pc, rd, rs1, imm) from the "decode" bus.
        lookups.push(self.register_lookup(
            Kind::Global(String::from("decode")),
            &vec![(
                local.pc.into_iter().map(Into::into)
                    .chain(once(F::from_u64(InstructionId::Srai as u64).into()))
        ...
        // Reuses the same "byte_srl" bus as SRL/SRA (same per-byte shift computation).
        lookups.extend(
            local
                .rs1_bytes
                .into_iter()
        ...
        // (0xFF << (8 - bit_shamt)) & 0xFF. We use the carry output as the
