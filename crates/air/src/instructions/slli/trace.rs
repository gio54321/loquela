...
use super::air::{SlliColumns, NUM_SLLI_COLS};
...
struct SlliStep {
    ...
    rd: u8,
    rs1: u8,
    /// Raw i32 immediate (bits 20–31 of instruction word, unsigned 5-bit shamt in low bits).
    imm: i32,
    rs1_value: u32,
    old_rd_value: u32,
    rd_new_value: u32,
...
fn extract_slli_steps(steps: &[ExecutionStep]) -> Vec<SlliStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rd, rs1, imm) = match step.instruction {
                Instruction::SlliI { rd, rs1, imm } => (rd, rs1, imm),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut SlliColumns<F>, step: &SlliStep) {
    ...
    // Compute per-byte shift results.
    let rs1_b = step.rs1_value.to_le_bytes();
    for i in 0..4 {
        let byte_val = rs1_b[i] as u32;
        let shifted = (byte_val << bit_shamt) & 0xFF;
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut SlliColumns<F>) {
    *row = SlliColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rd: F::ZERO,
...
/// Build the SLLI execution trace from the VM execution steps.
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    steps: &[ExecutionStep],
) -> DenseMatrix<F> {
    let slli_steps = extract_slli_steps(steps);
    assert!(!slli_steps.is_empty(), "no SLLI steps found in trace");

    let num_rows = slli_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_SLLI_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<SlliColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(slli_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(slli_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_SLLI_COLS)
}
