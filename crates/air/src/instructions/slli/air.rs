...
/// One row per SLLI instruction execution.
///
/// SLLI: `rd = rs1 << (imm & 0x1F)` (left shift, u32 wrapping).
///
/// I-type encoding: opcode=0x13, funct3=0x1, imm[11:5]=0b0000000.
...
///
/// Since `imm[11:5] == 0b0000000` is enforced by the decode AIR, `imm` is in 0..31.
/// So `shamt = imm` and no separate `shamt_high` check is needed.
///
...
///   - Sends `(InstructionId::Slli, rd, rs1, imm)` to the "decode" bus.
...
pub const NUM_SLLI_COLS: usize = size_of::<SlliColumns<u8>>();

impl<T> Borrow<SlliColumns<T>> for [T] {
    fn borrow(&self) -> &SlliColumns<T> {
        debug_assert_eq!(self.len(), NUM_SLLI_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<SlliColumns<T>>() };
...
impl<T> BorrowMut<SlliColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut SlliColumns<T> {
        debug_assert_eq!(self.len(), NUM_SLLI_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<SlliColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for SlliAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for SlliAir {
    fn width(&self) -> usize {
        NUM_SLLI_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for SlliAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &SlliColumns<AB::Var> = main.current_slice().borrow();
        ...
        // Since the decode AIR enforces imm[11:5]=0, imm is in 0..31, so
        // imm = shamt = bit_shamt + 8*byte_shamt with bit_shamt in 0..7, byte_shamt in 0..3.
        builder.assert_eq(
            local.imm.clone(),
...
impl<F: Field> LookupAir<F> for SlliAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &SlliColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        // Assert the decoded instruction is SLLI with (pc, rd, rs1, imm) from the "decode" bus.
        lookups.push(self.register_lookup(
            Kind::Global(String::from("decode")),
            &vec![(
                local.pc.into_iter().map(Into::into)
                    .chain(once(F::from_u64(InstructionId::Slli as u64).into()))
