...
/// One row per SB instruction execution.
///
/// SB stores only byte 0 of rs2 into RAM at address rs1 + imm.
/// The stored 32-bit RAM word has bytes 1–3 zeroed.
///
/// Wiring:
///   - Receives `(pc[0..4], timestamp)` from the "trace" bus.
///   - Sends `(InstructionId::Sb, rs1, rs2, imm_s)` to the "decode_s" bus.
///   - Sends three operations to the "memory" bus: read rs1, read rs2, write RAM.
///   - Sends byte-range tuples to the "bytes" bus for rs1_value, rs2_value byte 0, addr.
///   - Sends `(next_pc[0..4], timestamp + 3)` to the "trace" bus.
#[repr(C)]
pub struct SbColumns<F> {
    ...
    pub rs1_value: [F; 4],
    /// Value read from register `rs2` (full 32-bit, but only byte 0 is stored).
    pub rs2_value: [F; 4],
...
pub const NUM_SB_COLS: usize = size_of::<SbColumns<u8>>();

impl<T> Borrow<SbColumns<T>> for [T] {
    fn borrow(&self) -> &SbColumns<T> {
        debug_assert_eq!(self.len(), NUM_SB_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<SbColumns<T>>() };
...
impl<T> BorrowMut<SbColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut SbColumns<T> {
        debug_assert_eq!(self.len(), NUM_SB_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<SbColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for SbAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for SbAir {
    fn width(&self) -> usize {
        NUM_SB_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for SbAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &SbColumns<AB::Var> = main.current_slice().borrow();
...
impl<F: Field> LookupAir<F> for SbAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &SbColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        // Assert the decoded instruction is SB with (rs1, rs2, imm_s) from "decode_s" bus.
        lookups.push(self.register_lookup(
            Kind::Global(String::from("decode_s")),
            &vec![(
                once(F::from_u64(InstructionId::Sb as u64).into())
        ...
        // Write byte 0 of rs2 to RAM at addr, timestamp + 2.
        // The stored value is [rs2_value[0], 0, 0, 0].
        lookups.push(self.register_lookup(
            Kind::Global(String::from("memory")),
            &vec![(
                once((local.timestamp.clone() + F::from_u64(2)).into())
                    ...
                    .chain(
                        once(local.rs2_value[0].into())
                            .chain([F::ZERO.into(), F::ZERO.into(), F::ZERO.into()]),
                    )
                    .collect(),
                local.is_dummy.into(),
                Direction::Send,
        ...
        // Byte range-check for rs2_value byte 0 (the stored byte).
        lookups.push(self.register_lookup(
            Kind::Global(String::from("bytes")),
            &vec![(
                vec![local.rs2_value[0].into()],
                F::ONE.into(),
                Direction::Send,
            )],
        ));
