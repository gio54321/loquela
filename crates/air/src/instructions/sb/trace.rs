...
use super::air::{SbColumns, NUM_SB_COLS};
...
fn extract_sb_steps(steps: &[ExecutionStep]) -> Vec<SbStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rs1, rs2, imm) = match step.instruction {
                Instruction::Sb { rs1, rs2, imm } => (rs1, rs2, imm),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut SbColumns<F>, step: &SbStep) {
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut SbColumns<F>) {
    *row = SbColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rs1: F::ZERO,
...
/// Build the SB execution trace from the VM execution steps.
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    steps: &[ExecutionStep],
) -> DenseMatrix<F> {
    let sb_steps = extract_sb_steps(steps);
    assert!(!sb_steps.is_empty(), "no SB steps found in trace");

    let num_rows = sb_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_SB_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<SbColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(sb_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(sb_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_SB_COLS)
}
