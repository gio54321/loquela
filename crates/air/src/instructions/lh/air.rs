...
/// One row per LH instruction execution.
///
/// LH: `rd = sign_extend_16(MEM16[rs1 + imm])`.
///
/// The sign bit is bit 15 of the halfword = bit 7 of loaded_val[1].
///
/// Constraints:
///   - `rd_val[0] = loaded_val[0]`
///   - `rd_val[1] = loaded_val[1]`
///   - `loaded_val[1] = sign_bit * 128 + byte1_low7`
///   - `rd_val[2] = rd_val[3] = sign_bit * 0xFF`
#[repr(C)]
pub struct LhColumns<F> {
    pub pc: [F; 4],
    pub timestamp: F,
    pub rd: F,
    pub rs1: F,
    ...
    /// rd_val = sign_extend_16(loaded_val[0..2]).
    pub rd_val: [F; 4],

    /// Sign bit of the halfword: bit 7 of loaded_val[1].
    pub sign_bit: F,
    /// Low 7 bits of loaded_val[1]: `loaded_val[1] = sign_bit * 128 + byte1_low7`.
    pub byte1_low7: F,
...
pub const NUM_LH_COLS: usize = size_of::<LhColumns<u8>>();

impl<T> Borrow<LhColumns<T>> for [T] {
    fn borrow(&self) -> &LhColumns<T> {
        debug_assert_eq!(self.len(), NUM_LH_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<LhColumns<T>>() };
...
impl<T> BorrowMut<LhColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut LhColumns<T> {
        debug_assert_eq!(self.len(), NUM_LH_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<LhColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for LhAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for LhAir {
    fn width(&self) -> usize {
        NUM_LH_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for LhAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &LhColumns<AB::Var> = main.current_slice().borrow();
        ...
        // Sign bit extraction: loaded_val[1] = sign_bit * 128 + byte1_low7.
        builder.assert_bool(local.sign_bit.clone());
        builder.assert_eq(
            local.loaded_val[1].clone(),
            local.sign_bit.clone() * AB::Expr::from(AB::F::from_u32(128))
                + local.byte1_low7.clone(),
        );

        // rd_val[0] = loaded_val[0].
        builder.assert_eq(local.rd_val[0].clone(), local.loaded_val[0].clone());
        // rd_val[1] = loaded_val[1].
        builder.assert_eq(local.rd_val[1].clone(), local.loaded_val[1].clone());
        // Upper bytes are the sign extension.
        builder.assert_eq(
...
impl<F: Field> LookupAir<F> for LhAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &LhColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        lookups.push(self.register_lookup(
            ...
            &vec![(
                once(F::from_u64(InstructionId::Lh as u64).into())
        ...
        // Range-check byte1_low7 (must be in [0, 127]).
        lookups.push(self.register_lookup(
            Kind::Global(String::from("bytes")),
            &vec![(
                vec![local.byte1_low7.into()],
                F::ONE.into(),
                Direction::Send,
            )],
        ));
