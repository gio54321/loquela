...
use super::air::{ShColumns, NUM_SH_COLS};
...
fn extract_sh_steps(steps: &[ExecutionStep]) -> Vec<ShStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rs1, rs2, imm) = match step.instruction {
                Instruction::Sh { rs1, rs2, imm } => (rs1, rs2, imm),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut ShColumns<F>, step: &ShStep) {
    ...
    let se = (step.imm as u32).to_le_bytes();
    row.imm_se_bytes = [
    ...
    let carries = u32_add_carries(step.rs1_value, step.imm as u32);
    row.addr_carries = [
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut ShColumns<F>) {
    *row = ShColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rs1: F::ZERO,
...
/// Build the SH execution trace from the VM execution steps.
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    steps: &[ExecutionStep],
) -> DenseMatrix<F> {
    let sh_steps = extract_sh_steps(steps);
    assert!(!sh_steps.is_empty(), "no SH steps found in trace");

    let num_rows = sh_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_SH_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<ShColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(sh_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(sh_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_SH_COLS)
}
