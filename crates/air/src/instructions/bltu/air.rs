...
/// One row per BLTU instruction execution.
///
/// BLTU (B-type, opcode=0x63, funct3=0x6):
///   if rs1 < rs2 (unsigned), next_pc = pc + imm; else next_pc = pc + 4.
///
/// Unsigned comparison via borrow chain (same as SLTU):
///   taken = borrow_out[3]  (1 iff rs1 < rs2 unsigned).
#[repr(C)]
pub struct BltuColumns<F> {
    pub pc: [F; 4],
    pub timestamp: F,
    pub rs1: F,
    pub rs2: F,
    ...
    pub rs1_bytes: [F; 4],
    pub rs2_bytes: [F; 4],
    /// Byte-level difference rs1 - rs2 (wrapping).
    pub diff_bytes: [F; 4],
    /// Borrow-out bits for each byte of the subtraction.
    pub borrow: [F; 4],
    /// taken = borrow[3]: 1 iff rs1 < rs2 (unsigned).
    pub taken: F,
    pub next_pc: [F; 4],
    pub is_dummy: F,
}

pub const NUM_BLTU_COLS: usize = size_of::<BltuColumns<u8>>();

impl<T> Borrow<BltuColumns<T>> for [T] {
    fn borrow(&self) -> &BltuColumns<T> {
        debug_assert_eq!(self.len(), NUM_BLTU_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<BltuColumns<T>>() };
...
impl<T> BorrowMut<BltuColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut BltuColumns<T> {
        debug_assert_eq!(self.len(), NUM_BLTU_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<BltuColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for BltuAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for BltuAir {
    fn width(&self) -> usize {
        NUM_BLTU_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for BltuAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &BltuColumns<AB::Var> = main.current_slice().borrow();
        ...
        // taken = borrow[3] (unsigned lt).
        builder.assert_bool(local.taken.clone());
        builder.assert_eq(local.taken.clone(), local.borrow[3].clone());
...
impl<F: Field> LookupAir<F> for BltuAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &BltuColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        lookups.push(self.register_lookup(
            ...
            &vec![(
                vec![
                    F::from_u64(InstructionId::Bltu as u64).into(),
                    local.rs1.into(),
                    local.rs2.into(),
                    local.imm_top7.into(),
                    local.imm_lo5.into(),
