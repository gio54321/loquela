...
use super::air::{BltuColumns, NUM_BLTU_COLS};
...
fn extract_steps(steps: &[ExecutionStep]) -> Vec<BltuStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rs1, rs2, imm) = match step.instruction {
                Instruction::Bltu { rs1, rs2, imm } => (rs1, rs2, imm),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut BltuColumns<F>, step: &BltuStep) {
    ...
    // taken = 1 iff rs1 < rs2 (unsigned) = borrow[3].
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut BltuColumns<F>) {
    *row = BltuColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rs1: F::ZERO,
        ...
        diff_bytes: [F::ZERO; 4],
        borrow: [F::ZERO; 4],
        // taken=0: 0 < 0 is false, borrow[3]=0
        taken: F::ZERO,
...
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    ...
    assert!(!branch_steps.is_empty(), "no BLTU steps found in trace");

    let num_rows = branch_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_BLTU_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<BltuColumns<F>>() };
    ...
    DenseMatrix::new(values, NUM_BLTU_COLS)
}
