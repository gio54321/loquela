...
use super::air::{SrlColumns, NUM_SRL_COLS};
...
fn extract_srl_steps(steps: &[ExecutionStep]) -> Vec<SrlStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rd, rs1, rs2) = match step.instruction {
                Instruction::Srl { rd, rs1, rs2 } => (rd, rs1, rs2),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut SrlColumns<F>, step: &SrlStep) {
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut SrlColumns<F>) {
    *row = SrlColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rd: F::ZERO,
...
/// Build the SRL execution trace from the VM execution steps.
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    steps: &[ExecutionStep],
) -> DenseMatrix<F> {
    let srl_steps = extract_srl_steps(steps);
    assert!(!srl_steps.is_empty(), "no SRL steps found in trace");

    let num_rows = srl_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_SRL_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<SrlColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(srl_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(srl_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_SRL_COLS)
}
