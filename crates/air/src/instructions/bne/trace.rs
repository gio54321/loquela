...
use super::air::{BneColumns, NUM_BNE_COLS};
...
fn extract_bne_steps(steps: &[ExecutionStep]) -> Vec<BneStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rs1, rs2, imm) = match step.instruction {
                Instruction::Bne { rs1, rs2, imm } => (rs1, rs2, imm),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut BneColumns<F>, step: &BneStep) {
    ...
    for i in 0..4 {
        if diff_b[i] == 0 {
            ...
            row.diff_byte_inv[i] = F::from_u64(mod_inverse(diff_b[i] as u64, p));
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut BneColumns<F>) {
    *row = BneColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rs1: F::ZERO,
        ...
        // When rs1=rs2=0, diff=0, all byte_is_zero=1, taken=0 (BNE: taken when !=)
        byte_is_zero: [F::ONE; 4],
        taken: F::ZERO,
        // next_pc = pc_plus4 = 4 (not taken case)
...
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    ...
    let bne_steps = extract_bne_steps(steps);
    assert!(!bne_steps.is_empty(), "no BNE steps found in trace");

    let num_rows = bne_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_BNE_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<BneColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(bne_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(bne_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_BNE_COLS)
}
