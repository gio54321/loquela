...
/// One row per SLTI instruction execution.
///
/// SLTI: `rd = ((rs1 as i32) < sign_extend(imm)) ? 1 : 0` (signed comparison).
///
/// Comparison technique: compute `rs1 - imm_se` byte-by-byte with borrow chain,
/// then adjust for sign bits.
///
/// `slt = borrow XOR (sign_rs1 XOR sign_imm)`
///
/// where `sign_rs1 = rs1_bytes[3] >> 7`, `sign_imm = imm_se_bytes[3] >> 7 = imm[11]`.
///
/// Wiring:
///   - Receives `(pc[0..4], timestamp)` from the "trace" bus.
///   - Sends `(InstructionId::Slti, rd, rs1, imm)` to the "decode" bus.
///   - Sends two operations to the "memory" bus: read rs1, write rd.
///   - Sends byte range-checks for rs1_bytes, diff_bytes, rs1_byte3_low7 to the "bytes" bus.
///   - Sends `(next_pc[0..4], timestamp + 2)` to the "trace" bus.
#[repr(C)]
pub struct SltiColumns<F> {
    ...
    /// The signed less-than result: 1 iff (rs1 as i32) < sign_extend(imm).
    /// Equals `borrow[3] XOR (sign_rs1 XOR sign_imm)`.
    pub lt_result: F,
...
pub const NUM_SLTI_COLS: usize = size_of::<SltiColumns<u8>>();

impl<T> Borrow<SltiColumns<T>> for [T] {
    fn borrow(&self) -> &SltiColumns<T> {
        debug_assert_eq!(self.len(), NUM_SLTI_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<SltiColumns<T>>() };
...
impl<T> BorrowMut<SltiColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut SltiColumns<T> {
        debug_assert_eq!(self.len(), NUM_SLTI_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<SltiColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for SltiAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for SltiAir {
    fn width(&self) -> usize {
        NUM_SLTI_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for SltiAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &SltiColumns<AB::Var> = main.current_slice().borrow();
        ...
        let sign_imm: AB::Expr = local.imm_high_bits[3].clone().into();
        ...
        // Sign extension: byte 1 = high nibble + 0xF0 * sign_bit.
        builder.assert_eq(
            local.imm_se_bytes[1].clone(),
            imm_high_nibble + sign_imm.clone() * AB::Expr::from(AB::F::from_u32(0xF0)),
        );
        builder.assert_eq(
            local.imm_se_bytes[2].clone(),
            sign_imm.clone() * AB::Expr::from(AB::F::from_u32(0xFF)),
        );
        builder.assert_eq(
        ...
        // slt = borrow[3] XOR (sign_rs1 XOR sign_imm)
        // sign_imm = imm_high_bits[3] (already constrained boolean above).
        let sign_xor: AB::Expr = local.sign_rs1.clone() + sign_imm.clone()
            - local.sign_rs1.clone() * sign_imm * AB::Expr::TWO;
...
impl<F: Field> LookupAir<F> for SltiAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &SltiColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        // Assert the decoded instruction is SLTI with (pc, rd, rs1, imm) from the "decode" bus.
        lookups.push(self.register_lookup(
            Kind::Global(String::from("decode")),
            &vec![(
                local.pc.into_iter().map(Into::into)
                    .chain(once(F::from_u64(InstructionId::Slti as u64).into()))
