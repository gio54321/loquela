use loquela_vm::{ExecutionStep, Instruction, MemoryOperation};
use p3_field::PrimeCharacteristicRing;
use p3_matrix::dense::DenseMatrix;

use super::air::{SltiColumns, NUM_SLTI_COLS};

struct SltiStep {
    pc: u32,
    timestamp: u32,
    rd: u8,
    rs1: u8,
    imm: i32,
    rs1_value: u32,
    old_rd_value: u32,
}

fn extract_slti_steps(steps: &[ExecutionStep]) -> Vec<SltiStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rd, rs1, imm) = match step.instruction {
                Instruction::SltiI { rd, rs1, imm } => (rd, rs1, imm),
                _ => return None,
            };
            let (timestamp, rs1_value, old_rd_value) = match step.memory_ops.as_slice() {
                [MemoryOperation::Read {
                    timestamp,
                    value: v1,
                    ..
                }, MemoryOperation::Write { old_value, .. }] => (*timestamp, *v1, *old_value),
                _ => return None,
            };
            Some(SltiStep {
                pc: step.state.pc,
                timestamp,
                rd,
                rs1,
                imm,
                rs1_value,
                old_rd_value,
            })
        })
        .collect()
}

fn u32_to_limbs<F: PrimeCharacteristicRing>(v: u32) -> [F; 4] {
    let b = v.to_le_bytes();
    [
        F::from_u64(b[0] as u64),
        F::from_u64(b[1] as u64),
        F::from_u64(b[2] as u64),
        F::from_u64(b[3] as u64),
    ]
}

fn pc_plus4_carries(pc: u32) -> [u8; 3] {
    let b = pc.to_le_bytes();
    let s0 = b[0] as u32 + 4;
    let c0 = (s0 >> 8) as u8;
    let s1 = b[1] as u32 + c0 as u32;
    let c1 = (s1 >> 8) as u8;
    let s2 = b[2] as u32 + c1 as u32;
    let c2 = (s2 >> 8) as u8;
    [c0, c1, c2]
}

/// Compute borrow chain for rs1 - imm_se.
fn u32_sub_borrows(rs1: u32, rs2: u32) -> ([u8; 4], [u8; 4]) {
    let a = rs1.to_le_bytes();
    let b = rs2.to_le_bytes();
    let mut borrows = [0u8; 4];
    let mut diff = [0u8; 4];
    let mut borrow_in = 0u32;
    for i in 0..4 {
        let sub = a[i] as i32 - b[i] as i32 - borrow_in as i32;
        if sub < 0 {
            diff[i] = (sub + 256) as u8;
            borrows[i] = 1;
        } else {
            diff[i] = sub as u8;
            borrows[i] = 0;
        }
        borrow_in = borrows[i] as u32;
    }
    (diff, borrows)
}

fn fill_row<F: PrimeCharacteristicRing>(row: &mut SltiColumns<F>, step: &SltiStep) {
    row.pc = u32_to_limbs(step.pc);
    row.timestamp = F::from_u64(step.timestamp as u64);
    row.rd = F::from_u64(step.rd as u64);
    row.rs1 = F::from_u64(step.rs1 as u64);

    // The 12-bit unsigned immediate (bits 20..31 of instruction word).
    let imm_u12 = (step.imm as u32) & 0xFFF;
    row.imm = F::from_u64(imm_u12 as u64);

    // Sign-extended immediate (as u32).
    let imm_se = step.imm as u32;

    // High nibble = bits 8..11 of the 12-bit immediate.
    let imm_high_nibble = (imm_u12 >> 8) & 0xF;
    row.imm_high_bits = [
        F::from_u64((imm_high_nibble & 1) as u64),
        F::from_u64(((imm_high_nibble >> 1) & 1) as u64),
        F::from_u64(((imm_high_nibble >> 2) & 1) as u64),
        F::from_u64(((imm_high_nibble >> 3) & 1) as u64),
    ];

    row.imm_se_bytes = u32_to_limbs(imm_se);

    row.rs1_bytes = u32_to_limbs(step.rs1_value);
    row.old_rd_value = u32_to_limbs(step.old_rd_value);

    let (diff, borrows) = u32_sub_borrows(step.rs1_value, imm_se);
    row.diff_bytes = diff.map(|b| F::from_u64(b as u64));
    row.borrow = borrows.map(|b| F::from_u64(b as u64));

    let sign_rs1 = (step.rs1_value >> 31) as u8;
    row.sign_rs1 = F::from_u64(sign_rs1 as u64);

    let rs1_high_byte = ((step.rs1_value >> 24) & 0xFF) as u8;
    row.rs1_byte3_low7 = F::from_u64((rs1_high_byte & 0x7F) as u64);

    let lt_result = if (step.rs1_value as i32) < step.imm {
        F::ONE
    } else {
        F::ZERO
    };
    row.lt_result = lt_result;

    row.is_dummy = F::ONE;

    row.next_pc = u32_to_limbs(step.pc + 4);
    let carries = pc_plus4_carries(step.pc);
    row.next_pc_carries = [
        F::from_u64(carries[0] as u64),
        F::from_u64(carries[1] as u64),
        F::from_u64(carries[2] as u64),
    ];
}

fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut SltiColumns<F>) {
    *row = SltiColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rd: F::ZERO,
        rs1: F::ZERO,
        imm: F::ZERO,
        imm_high_bits: [F::ZERO; 4],
        imm_se_bytes: [F::ZERO; 4],
        rs1_bytes: [F::ZERO; 4],
        old_rd_value: [F::ZERO; 4],
        diff_bytes: [F::ZERO; 4],
        borrow: [F::ZERO; 4],
        sign_rs1: F::ZERO,
        rs1_byte3_low7: F::ZERO,
        lt_result: F::ZERO,
        next_pc: [F::from_u64(4), F::ZERO, F::ZERO, F::ZERO],
        next_pc_carries: [F::ZERO; 3],
        is_dummy: F::ZERO,
    };
}

/// Build the SLTI execution trace from the VM execution steps.
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    steps: &[ExecutionStep],
) -> DenseMatrix<F> {
    let slti_steps = extract_slti_steps(steps);
    assert!(!slti_steps.is_empty(), "no SLTI steps found in trace");

    let num_rows = slti_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_SLTI_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<SltiColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(slti_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(slti_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_SLTI_COLS)
}
