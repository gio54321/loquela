...
/// One row per BLT instruction execution.
///
/// BLT (B-type, opcode=0x63, funct3=0x4):
///   if (rs1 as i32) < (rs2 as i32), next_pc = pc + imm; else next_pc = pc + 4.
///
/// Signed comparison via borrow chain + sign XOR (same as SLT):
///   slt = borrow[3] XOR (sign_rs1 XOR sign_rs2)
///   taken = slt.
#[repr(C)]
pub struct BltColumns<F> {
    pub pc: [F; 4],
    pub timestamp: F,
    pub rs1: F,
    pub rs2: F,
    ...
    pub diff_bytes: [F; 4],
    pub borrow: [F; 4],
    /// Sign bit of rs1 (bit 31).
    pub sign_rs1: F,
    /// Sign bit of rs2 (bit 31).
    pub sign_rs2: F,
    /// Low 7 bits of rs1_bytes[3].
    pub rs1_byte3_low7: F,
    /// Low 7 bits of rs2_bytes[3].
    pub rs2_byte3_low7: F,
    /// taken = slt = borrow[3] XOR (sign_rs1 XOR sign_rs2).
    pub taken: F,
    pub next_pc: [F; 4],
    pub is_dummy: F,
}

pub const NUM_BLT_COLS: usize = size_of::<BltColumns<u8>>();

impl<T> Borrow<BltColumns<T>> for [T] {
    fn borrow(&self) -> &BltColumns<T> {
        debug_assert_eq!(self.len(), NUM_BLT_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<BltColumns<T>>() };
...
impl<T> BorrowMut<BltColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut BltColumns<T> {
        debug_assert_eq!(self.len(), NUM_BLT_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<BltColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for BltAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for BltAir {
    fn width(&self) -> usize {
        NUM_BLT_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for BltAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &BltColumns<AB::Var> = main.current_slice().borrow();
        ...
        builder.assert_eq(local.taken.clone(), slt);
...
impl<F: Field> LookupAir<F> for BltAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &BltColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        lookups.push(self.register_lookup(
            ...
            &vec![(
                vec![
                    F::from_u64(InstructionId::Blt as u64).into(),
                    local.rs1.into(),
                    local.rs2.into(),
                    local.imm_top7.into(),
                    local.imm_lo5.into(),
        ...
        // Range-check rs1_byte3_low7 and rs2_byte3_low7.
        lookups.push(self.register_lookup(
            Kind::Global(String::from("bytes")),
            &vec![(
