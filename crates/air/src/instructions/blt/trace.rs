...
use super::air::{BltColumns, NUM_BLT_COLS};
...
fn extract_steps(steps: &[ExecutionStep]) -> Vec<BltStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rs1, rs2, imm) = match step.instruction {
                Instruction::Blt { rs1, rs2, imm } => (rs1, rs2, imm),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut BltColumns<F>, step: &BltStep) {
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut BltColumns<F>) {
    *row = BltColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rs1: F::ZERO,
...
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    ...
    assert!(!branch_steps.is_empty(), "no BLT steps found in trace");

    let num_rows = branch_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_BLT_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<BltColumns<F>>() };
    ...
    DenseMatrix::new(values, NUM_BLT_COLS)
}
