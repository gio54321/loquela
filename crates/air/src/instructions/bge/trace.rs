...
use super::air::{BgeColumns, NUM_BGE_COLS};
...
fn extract_steps(steps: &[ExecutionStep]) -> Vec<BgeStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rs1, rs2, imm) = match step.instruction {
                Instruction::Bge { rs1, rs2, imm } => (rs1, rs2, imm),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut BgeColumns<F>, step: &BgeStep) {
    ...
    let sign_rs1 = (step.rs1_value >> 31) & 1;
    let sign_rs2 = (step.rs2_value >> 31) & 1;
    row.sign_rs1 = F::from_u64(sign_rs1 as u64);
    row.sign_rs2 = F::from_u64(sign_rs2 as u64);
    row.rs1_byte3_low7 = F::from_u64(((step.rs1_value >> 24) & 0x7F) as u64);
    row.rs2_byte3_low7 = F::from_u64(((step.rs2_value >> 24) & 0x7F) as u64);

    // taken = 1 iff (rs1 as i32) >= (rs2 as i32).
    let taken = (step.rs1_value as i32) >= (step.rs2_value as i32);
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut BgeColumns<F>) {
    *row = BgeColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rs1: F::ZERO,
        ...
        // taken=1: 0 >= 0 is true, slt=0, taken=1-0=1
        taken: F::ONE,
        next_pc: [F::ZERO; 4],
        is_dummy: F::ZERO,
    };
...
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    ...
    assert!(!branch_steps.is_empty(), "no BGE steps found in trace");

    let num_rows = branch_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_BGE_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<BgeColumns<F>>() };
    ...
    DenseMatrix::new(values, NUM_BGE_COLS)
}
