...
use super::air::{SrliColumns, NUM_SRLI_COLS};
...
fn extract_srli_steps(steps: &[ExecutionStep]) -> Vec<SrliStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rd, rs1, imm) = match step.instruction {
                Instruction::SrliI { rd, rs1, imm } => (rd, rs1, imm),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut SrliColumns<F>, step: &SrliStep) {
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut SrliColumns<F>) {
    *row = SrliColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rd: F::ZERO,
...
/// Build the SRLI execution trace from the VM execution steps.
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    steps: &[ExecutionStep],
) -> DenseMatrix<F> {
    let srli_steps = extract_srli_steps(steps);
    assert!(!srli_steps.is_empty(), "no SRLI steps found in trace");

    let num_rows = srli_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_SRLI_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<SrliColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(srli_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(srli_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_SRLI_COLS)
}
