...
pub const NUM_OR_COLS: usize = size_of::<OrColumns<u8>>();

impl<T> Borrow<OrColumns<T>> for [T] {
    fn borrow(&self) -> &OrColumns<T> {
        debug_assert_eq!(self.len(), NUM_OR_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<OrColumns<T>>() };
...
impl<T> BorrowMut<OrColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut OrColumns<T> {
        debug_assert_eq!(self.len(), NUM_OR_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<OrColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for OrInstrAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for OrInstrAir {
    fn width(&self) -> usize {
        NUM_OR_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for OrInstrAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &OrColumns<AB::Var> = main.current_slice().borrow();
...
impl<F: Field> LookupAir<F> for OrInstrAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &OrColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        // Assert the decoded instruction is OR with (pc, rd, rs1, rs2) from the "decode" bus.
        lookups.push(self.register_lookup(
            Kind::Global(String::from("decode")),
            &vec![(
                local.pc.into_iter().map(Into::into)
                    .chain(once(F::from_u64(InstructionId::Or as u64).into()))
        ...
        // Write the OR result to rd at timestamp + 2.
        lookups.push(self.register_lookup(
            Kind::Global(String::from("memory")),
            &vec![(
        ...
        // Four byte-level OR lookups: rd_new[i] = rs1_value[i] | rs2_value[i].
