...
use super::air::{OrColumns, NUM_OR_COLS};
...
fn extract_or_steps(steps: &[ExecutionStep]) -> Vec<OrStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rd, rs1, rs2) = match step.instruction {
                Instruction::Or { rd, rs1, rs2 } => (rd, rs1, rs2),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut OrColumns<F>, step: &OrStep) {
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut OrColumns<F>) {
    *row = OrColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rd: F::ZERO,
...
/// Build the OR execution trace from the VM execution steps.
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    steps: &[ExecutionStep],
) -> DenseMatrix<F> {
    let or_steps = extract_or_steps(steps);
    assert!(!or_steps.is_empty(), "no OR steps found in trace");

    let num_rows = or_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_OR_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<OrColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(or_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(or_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_OR_COLS)
}
