...
/// One row per ORI instruction execution.
///
/// Wiring:
///   - Receives `(pc[0..4], timestamp)` from the "trace" bus (current execution state).
///   - Sends `(InstructionId::Ori, rd, rs1, imm)` to the "decode" bus.
...
pub const NUM_ORI_COLS: usize = size_of::<OriColumns<u8>>();

impl<T> Borrow<OriColumns<T>> for [T] {
    fn borrow(&self) -> &OriColumns<T> {
        debug_assert_eq!(self.len(), NUM_ORI_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<OriColumns<T>>() };
...
impl<T> BorrowMut<OriColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut OriColumns<T> {
        debug_assert_eq!(self.len(), NUM_ORI_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<OriColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for OriAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for OriAir {
    fn width(&self) -> usize {
        NUM_ORI_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for OriAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &OriColumns<AB::Var> = main.current_slice().borrow();
...
impl<F: Field> LookupAir<F> for OriAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &OriColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        // Assert the decoded instruction is ORI with (pc, rd, rs1, imm) from the "decode" bus.
        lookups.push(self.register_lookup(
            Kind::Global(String::from("decode")),
            &vec![(
                local.pc.into_iter().map(Into::into)
                    .chain(once(F::from_u64(InstructionId::Ori as u64).into()))
        ...
        // Write the OR result to register rd at timestamp + 1.
        lookups.push(self.register_lookup(
            Kind::Global(String::from("memory")),
            &vec![(
        ...
        // Four byte-level OR lookups: rd_new[i] = rs1_value[i] | imm_se_bytes[i].
