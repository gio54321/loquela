...
use super::air::{SllColumns, NUM_SLL_COLS};
...
fn extract_sll_steps(steps: &[ExecutionStep]) -> Vec<SllStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rd, rs1, rs2) = match step.instruction {
                Instruction::Sll { rd, rs1, rs2 } => (rd, rs1, rs2),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut SllColumns<F>, step: &SllStep) {
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut SllColumns<F>) {
    *row = SllColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rd: F::ZERO,
...
/// Build the SLL execution trace from the VM execution steps.
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    steps: &[ExecutionStep],
) -> DenseMatrix<F> {
    let sll_steps = extract_sll_steps(steps);
    assert!(!sll_steps.is_empty(), "no SLL steps found in trace");

    let num_rows = sll_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_SLL_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<SllColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(sll_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(sll_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_SLL_COLS)
}
