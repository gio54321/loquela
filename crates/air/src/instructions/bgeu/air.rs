...
/// One row per BGEU instruction execution.
///
/// BGEU (B-type, opcode=0x63, funct3=0x7):
///   if rs1 >= rs2 (unsigned), next_pc = pc + imm; else next_pc = pc + 4.
///   taken = 1 - borrow[3]  (1 iff rs1 >= rs2 unsigned).
#[repr(C)]
pub struct BgeuColumns<F> {
    pub pc: [F; 4],
    pub timestamp: F,
    pub rs1: F,
    pub rs2: F,
...
pub const NUM_BGEU_COLS: usize = size_of::<BgeuColumns<u8>>();

impl<T> Borrow<BgeuColumns<T>> for [T] {
    fn borrow(&self) -> &BgeuColumns<T> {
        debug_assert_eq!(self.len(), NUM_BGEU_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<BgeuColumns<T>>() };
...
impl<T> BorrowMut<BgeuColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut BgeuColumns<T> {
        debug_assert_eq!(self.len(), NUM_BGEU_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<BgeuColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for BgeuAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for BgeuAir {
    fn width(&self) -> usize {
        NUM_BGEU_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for BgeuAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &BgeuColumns<AB::Var> = main.current_slice().borrow();
        ...
        // taken = 1 - borrow[3] (unsigned ge).
        builder.assert_bool(local.taken.clone());
        builder.assert_eq(local.taken.clone(), AB::Expr::ONE - local.borrow[3].clone());
...
impl<F: Field> LookupAir<F> for BgeuAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &BgeuColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        lookups.push(self.register_lookup(
            ...
            &vec![(
                vec![
                    F::from_u64(InstructionId::Bgeu as u64).into(),
                    local.rs1.into(),
                    local.rs2.into(),
                    local.imm_top7.into(),
                    local.imm_lo5.into(),
