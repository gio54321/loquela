...
use super::air::{LwColumns, NUM_LW_COLS};
...
fn extract_lw_steps(steps: &[ExecutionStep]) -> Vec<LwStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rd, rs1, imm) = match step.instruction {
                Instruction::Lw { rd, rs1, imm } => (rd, rs1, imm),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut LwColumns<F>, step: &LwStep) {
    ...
    // Store the unsigned 12-bit immediate.
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut LwColumns<F>) {
    *row = LwColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rd: F::ZERO,
...
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    ...
    let lw_steps = extract_lw_steps(steps);
    assert!(!lw_steps.is_empty(), "no LW steps found in trace");

    let num_rows = lw_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_LW_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<LwColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(lw_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(lw_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_LW_COLS)
}
