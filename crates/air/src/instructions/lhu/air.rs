...
/// One row per LHU instruction execution.
///
/// LHU: `rd = MEM16[rs1 + imm] & 0xFFFF` (unsigned halfword load, zero-extended).
///
/// rd_val = [loaded_val[0], loaded_val[1], 0, 0].
#[repr(C)]
pub struct LhuColumns<F> {
    pub pc: [F; 4],
    pub timestamp: F,
    pub rd: F,
    pub rs1: F,
...
pub const NUM_LHU_COLS: usize = size_of::<LhuColumns<u8>>();

impl<T> Borrow<LhuColumns<T>> for [T] {
    fn borrow(&self) -> &LhuColumns<T> {
        debug_assert_eq!(self.len(), NUM_LHU_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to::<LhuColumns<T>>() };
...
impl<T> BorrowMut<LhuColumns<T>> for [T] {
    fn borrow_mut(&mut self) -> &mut LhuColumns<T> {
        debug_assert_eq!(self.len(), NUM_LHU_COLS);
        let (prefix, shorts, suffix) = unsafe { self.align_to_mut::<LhuColumns<T>>() };
        debug_assert!(prefix.is_empty(), "Alignment should match");
        debug_assert!(suffix.is_empty(), "Alignment should match");
        debug_assert_eq!(shorts.len(), 1);
        &mut shorts[0]
...
impl Default for LhuAir {
    fn default() -> Self {
        Self::new()
    }
}

impl<F> BaseAir<F> for LhuAir {
    fn width(&self) -> usize {
        NUM_LHU_COLS
    }
}

impl<AB: AirBuilder> Air<AB> for LhuAir
where
    ...
    AB::F: PrimeCharacteristicRing + QuotientMap<u32>,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &LhuColumns<AB::Var> = main.current_slice().borrow();
        ...
        // LHU: rd_val = [loaded_val[0], loaded_val[1], 0, 0].
...
impl<F: Field> LookupAir<F> for LhuAir {
    ...
    fn get_lookups(&mut self) -> Vec<Lookup<F>> {
        ...
        let local: &LhuColumns<SymbolicVariable<F>> = symbolic_main.current_slice().borrow();
        ...
        lookups.push(self.register_lookup(
            ...
            &vec![(
                once(F::from_u64(InstructionId::Lhu as u64).into())
