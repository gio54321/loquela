...
use super::air::{LhuColumns, NUM_LHU_COLS};
...
fn extract_lhu_steps(steps: &[ExecutionStep]) -> Vec<LhuStep> {
    steps
        .iter()
        .filter_map(|step| {
            let (rd, rs1, imm) = match step.instruction {
                Instruction::Lhu { rd, rs1, imm } => (rd, rs1, imm),
                _ => return None,
            };
...
fn fill_row<F: PrimeCharacteristicRing>(row: &mut LhuColumns<F>, step: &LhuStep) {
...
fn fill_padding_row<F: PrimeCharacteristicRing>(row: &mut LhuColumns<F>) {
    *row = LhuColumns {
        pc: [F::ZERO; 4],
        timestamp: F::ZERO,
        rd: F::ZERO,
...
pub fn build_trace<F: PrimeCharacteristicRing + Send + Sync>(
    ...
    let lhu_steps = extract_lhu_steps(steps);
    assert!(!lhu_steps.is_empty(), "no LHU steps found in trace");

    let num_rows = lhu_steps.len().next_power_of_two().max(4);
    let mut values = vec![F::ZERO; num_rows * NUM_LHU_COLS];

    let (prefix, rows, suffix) = unsafe { values.align_to_mut::<LhuColumns<F>>() };
    assert!(prefix.is_empty(), "alignment mismatch");
    assert!(suffix.is_empty(), "alignment mismatch");
    assert_eq!(rows.len(), num_rows);

    for (row, step) in rows.iter_mut().zip(lhu_steps.iter()) {
        fill_row(row, step);
    }
    for row in rows.iter_mut().skip(lhu_steps.len()) {
        fill_padding_row(row);
    }

    DenseMatrix::new(values, NUM_LHU_COLS)
}
